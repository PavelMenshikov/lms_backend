...
type LearningRepository interface {
	...
	GetTeacherMonthlyReport(ctx context.Context, teacherID string, year, month int) (*domain.TeacherMonthlyReport, error)
}
...
func (r *LearningRepoImpl) GetTeacherMonthlyReport(ctx context.Context, teacherID string, year, month int) (*domain.TeacherMonthlyReport, error) {
	...
	_ = r.db.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM lessons
		WHERE substituted_teacher_id = $1
		  AND lesson_time >= $2::date AND lesson_time < $3::date
		  AND is_cancelled = FALSE
	...
	_ = r.db.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM lessons
		WHERE teacher_id = $1 AND substituted_teacher_id IS NOT NULL
		  AND lesson_time >= $2::date AND lesson_time < $3::date
