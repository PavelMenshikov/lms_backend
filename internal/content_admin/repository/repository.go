...
func (r *ContentAdminRepoImpl) UpdateCourseSettings(ctx context.Context, course *domain.Course) error {
	query := `
		UPDATE courses 
		SET has_homework = $1, is_homework_mandatory = $2, is_test_mandatory = $3,
			is_project_mandatory = $4, is_discord_mandatory = $5, is_anti_copy_enabled = $6,
			status = $7, title = $8, description = $9, image_url = $10, is_main = $11
		WHERE id = $12
	`
	_, err := r.db.ExecContext(ctx, query,
		course.HasHomework, course.IsHomeworkMandatory, course.IsTestMandatory,
		course.IsProjectMandatory, course.IsDiscordMandatory, course.IsAntiCopyEnabled,
		course.Status, course.Title, course.Description, course.ImageURL, course.IsMain,
		course.ID,
	)
	return err
}
...
func (r *ContentAdminRepoImpl) CreateLesson(ctx context.Context, lesson *domain.Lesson) (string, error) {
	...
	err := r.db.QueryRowContext(ctx, query,
		lesson.ModuleID, lesson.TeacherID, lesson.Title, lesson.LessonTime,
		lesson.OrderNum, lesson.VideoURL, lesson.PresentationURL, lesson.ContentText,
		lesson.IsPublished,
	).Scan(&newID)
	if err != nil {
...
func (r *ContentAdminRepoImpl) CreateUser(ctx context.Context, user *domain.User) (string, error) {
	var newID string
	query := `INSERT INTO users (first_name, last_name, email, password_hash, role) VALUES ($1, $2, $3, $4, $5) RETURNING id`
	err := r.db.QueryRowContext(ctx, query, user.FirstName, user.LastName, user.Email, user.Password, user.Role).Scan(&newID)
	if err != nil {
...
func (r *ContentAdminRepoImpl) EnrollStudent(ctx context.Context, userID, courseID string) error {
	query := `INSERT INTO user_courses (user_id, course_id, progress_percent) VALUES ($1, $2, 0) ON CONFLICT (user_id, course_id) DO NOTHING`
